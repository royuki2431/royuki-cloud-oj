...
@Slf4j
@Component
public class CSandbox implements LanguageSandbox {
    ...
    public JudgeResult judge(String code, List<JudgeTestCase> testCases, int timeLimit, int memoryLimit) {
        ...
        try {
            ...
            log.info("C语言评测开始, workDir={}", workDir);
            ...
            // 编译代码
            DockerSandbox.DockerExecuteResult compileResult = compile(workDir, timeLimit);
            ...
            int passedCount = 0;
            long totalTime = compileResult.getExecuteTime();
            long maxMemory = 0;
            ...
            log.info("C语言评测完成: status={}, score={}, passed={}/{}", 
    ...
    private DockerSandbox.DockerExecuteResult compile(String workDir, int timeLimit) {
        String[] command = new String[]{
                "sh", "-c",
                "gcc -std=c11 -O2 -o " + EXECUTABLE_FILE + " " + SOURCE_FILE + " 2>&1"
        };
